...
### 0:20 – 0:45 · The idea in one line
...
### 0:45 – 2:00 · Incident #1 — the retry tax (live heal)
Run it:
```bash
python self_heal.py
```
...
> it's allowed. Then it rolls out again and re-queries SigNoz to verify."
...
### 2:00 – 2:40 · Incident #2 — the bill-shock kill-switch (the money shot)
Run it:
```bash
...
### 2:40 – 3:00 · Why SigNoz, and close
...
CPU inference makes each `llm.chat` 10–110s, so a full cycle is a few minutes.
Options:
