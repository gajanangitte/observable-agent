...
| `heal_grounding.py` | **Groundedness auditor**: scores every autonomous decision HIGH/MEDIUM/LOW/NONE from the evidence it read, whether the action actually remediates the breached SLO, and known fingerprint recall; stamps `heal.grounding.*` on the trace and caps autonomy to suggest when a decision is ungrounded |
| `heal_ledger.py` | **Tamper-evident audit ledger**: append only, SHA-256 hash chained record of every breach, action, verify, and outcome; `verify_chain` proves the trail was not edited, removed, or reordered (a live INTACT check rides on every heal) |
| `chaos.py` | **Named chaos catalog**: a deterministic fault library (`response_drop`, `runaway_loop`, `carbon_waste`), each tied to a real injection point, with a CLI to list, describe, inject, status, and clear so a breach is reproducible on demand |
| `eval.py` | **Offline chaos and safety harness**: replays scripted adversarial incidents through the real detection, policy gate, memory, and rollback code with a scripted `FakeMCP`, scores classification accuracy, and asserts the one invariant that matters (zero unsafe actions); `--no-export` needs no SigNoz, runs in about 7 seconds, and exits non-zero on any misclassification (CI gate) |
| `version.py` | **Deploy and version correlation**: resolves the running build from the git SHA (with a `+dirty` marker) and feeds `service.version` + `deployment.commit`/`branch`/`dirty` into the OTel Resource, so every span says exactly which build produced it |
| `console.py` | **Zero dependency status console**: a read-only stdlib web panel (no framework, no new deps) that shows the build, armed faults, verified memory, the ledger integrity check, and the last Track 02/03 verdicts at a glance next to SigNoz |
| `self_debug.py` | **The agent turned on itself**: hands the same local model a curated set of SigNoz MCP tools and asks it to debug its OWN latency, rooted in an `agent.introspect` span whose children are real `mcp.signoz_*` calls, so the self debugging session is itself a trace, the agent observing the agent |
| `introspect_tools.py` | The three argument free self observability tools `self_debug.py` exposes to the model; each maps to a real SigNoz MCP `tools/call` under its own `mcp.signoz_*` span, pinning the correct arguments and trimming the JSON so a small CPU model can reason about its own telemetry |
...
| `mcp2_proxy.py` / `mcp2_proxy_core.py` | **Always on MCP proxy**: a transparent streamable-HTTP passthrough that auto-instruments ANY MCP client and server (spans + `mcp.client.*` metrics on service `mcp-proxy`), zero code changes to either side; the pure core (JSON-RPC + SSE parsing, outcome grading) has 24 network free tests |
...
| `tests/` | 289 network free tests (`python tests/run_all.py`) covering the detection, policy gate, ledger, economics, energy, accessibility, and MCP certification cores; no SigNoz and no model required |
...
| `docs/TRACK02_MCP2.md` | **MCP Contract Lab** writeup: observability as tests for the MCP protocol, the eight contracts, the three signals, the alerts, and the always on auto instrumenting proxy |
